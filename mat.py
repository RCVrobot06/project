...
def get_color_from_cmap(index):
    # colormap에서 색상 값을 가져오기
    color = tab20c_cmap(index % tab20c_cmap.N)
    r, g, b = [int(255 * c) for c in color[:3]]  # RGB로 변환하고 0-255로 스케일 조정
    return r, g, b
    ...
    for i, detection in enumerate(detections):
        ...
        if valid_contours:
            ...
            if obj_id not in color_map:
                # colormap에서 색상 가져오기
                color_map[obj_id] = get_color_from_cmap(len(color_map))
